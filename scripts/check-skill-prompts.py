...
SPECS = {
    ...
    "done": {
        ...
        "contracts": {
            ...
            "1. Full verify": ("green after the last code edit", "Run it only when no such result exists",
                               "Red → you are not here yet.", "No verify command exists"),
            ...
            "3. Risk-routed independent review": ("`security`", "`contract`", "`general`",
                                                   "built-in review command", "no review is the default", "run **one** pass", "never two",
    ...
    "lucia-relay": {
        ...
        "contracts": {
            ...
            "Produce": ("commit and push every task file first", "--recipient cross-machine --base <base> --write",
                        "publishes a commit-named transfer tag", "sanitized clone URL", "committed changed files",
                        "refuses to replace an existing `LUCIA_RELAY.json`",
                        ...
                        "exclude credentials", "relay.py finalize --root .", "prints the trusted envelope",
                        "`--envelope-out <file>` saves it outside the repository",
            ...
            "Receive": ("Obtain the trusted envelope.", "detached is valid",
                        "keep the envelope outside the clone",
        ...
        "code": (("Produce", "relay.py draft --root . --recipient same-machine --write"),
                 ("Produce", "relay.py draft --root . --recipient cross-machine --base <base> --write"),
                 ("Produce", "relay.py finalize --root ."),
                 ("Receive", "relay.py inspect --root . --trusted-envelope <file>"),
