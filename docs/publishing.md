...
below were verified 2026-08-10; re-check anything marked (†) before relying
...
- [ ] Cut a release per CONTRIBUTING: move `[Unreleased]` into a new dated
      version heading, bump `.claude-plugin/plugin.json` + `package.json` to
      that same version (`test.sh` fails on any mismatch), then `git tag vX.Y.Z`
- [x] `repository`/`homepage` fields added to `package.json` and
      `.claude-plugin/plugin.json` — done 2026-08-10
- [ ] `./test.sh` green; `claude plugin validate .` passes
...
- [x] Configure **trusted publishing** (OIDC) — registered 2026-08-10:
      ...
      environment blank. Will be exercised (and provenance-verified via
      `npm view luciazero dist.attestations`) by the next tag; attestations
      are generated by default under OIDC, no `--provenance` flag needed (†)
