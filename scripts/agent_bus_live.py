...
def cli_env(daemon: Daemon) -> dict[str, str]:
    ...
    return dict(os.environ, PYTHONDONTWRITEBYTECODE="1", PYTHONPATH=str(ROOT / "agentd"),
                LUCIAZERO_AGENT_BUS_HOME=str(daemon.state_dir))
...
def enrol(daemon: Daemon, provider: str, binary: str, cwd: Path, timeout: int) -> None:
    ...
    argv = [sys.executable, "-m", "luciazero_agentd", "worker", "add", WORKERS[provider], provider,
            "--cwd", str(cwd), "--max-attempts", "1", "--timeout", str(timeout),
            "--approve", POLICY, "--state-dir", str(daemon.state_dir), "--", binary]
...
def check_turn(daemon: Daemon, provider: str, work: dict[str, Any], turn_dir: Path) -> dict[str, Any]:
    ...
    with store_of(daemon) as store:
        ...
        while True:
            ...
            if not page:
                break
            events.extend(e for e in page if e.get("actor_agent_id") == worker)
            after = page[-1]["seq"]
        failures = []
        ...
        if not any(e["payload"].get("trust") == "bound" for e in events):
            failures.append("no write from the worker was recorded as bound")
        body = log_tail(run)
        if failures:
...
def one_provider(daemon: Daemon, provider: str, binary: str, root: Path, run: str, timeout: int, narrate) -> dict[str, Any]:
    worker = WORKERS[provider]
    cwd = root / f"work-{provider}"
    cwd.mkdir(parents=True, exist_ok=True)
    agentd(daemon, "roster", "add", worker, provider, "worker", "--capability", "bus")
    enrol(daemon, provider, binary, cwd, timeout)
    narrate(f"#   {worker} enrolled: {binary} (approvals: {POLICY}, one attempt, {timeout}s)")
    work = queue_work(daemon, provider, run)
    narrate(f"#   one task queued; starting the turn (this spends {provider} quota)")
...
def main() -> int:
    ...
    binaries = {provider: shutil.which(provider) for provider in wanted}
    missing = [provider for provider, path in binaries.items() if path is None]
    if missing:
    ...
    try:
        ...
        for provider in wanted:
            ...
            result = one_provider(daemon, provider, str(binaries[provider]), root, run, args.timeout, narrate)
    ...
    print(f"PASS  agent bus M6 live smoke gate ({', '.join(r['provider'] for r in results)})")
    return 0
