...
./test.sh --agent-bus-store      # 358 tests, includes the dispatch, adapter, watcher and claim suites
...
- [x] The daemon prints a one-time code on **its own console**, and
  `luciazero-agentd claim approve <id> --code <code>` is how a person answers.
  The first version relied on a process-ancestry check instead; the security
  review broke it from inside a live session with `( … & )` plus `script`,
  because ancestry cannot be a boundary against a process that can fork. Five
  wrong codes kill the request.
- [x] Approving re-checks that the agent is still unbound, so a stale request
  cannot revoke a terminal the user bound in the meantime, and the binding is
  revoked when the MCP session ends or is evicted rather than stranding a
  phantom terminal for hours.
...
- [x] Regressions (33), red-first on six: the code being required, guesses
  running out, an approval binding some other session, a stale claim taking a
  live terminal, two claims for one agent, and the identity outliving its MCP
  session. Reviewed adversarially; one blocker and two majors were found and
